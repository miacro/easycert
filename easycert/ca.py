...
class Pathname():
    def __init__(self, prefix="~/.CA", infix="root", basename="ROOT_CA"):
    ...
    def get_keyfile(self, *args, **kwargs):
        return self.get_pathname("private", ".key.pem", *args, **kwargs)

    def get_crlfile(self, *args, **kwargs):
        return self.get_pathname("crl", ".crl.pem", *args, **kwargs)

    def get_csrfile(self, *args, **kwargs):
        return self.get_pathname("csr", ".csr.pem", *args, **kwargs)

    def get_crtfile(self, *args, **kwargs):
        return self.get_pathname("certs", ".crt.pem", *args, **kwargs)

    def get_cachainfile(self, *args, **kwargs):
        return self.get_pathname("certs", ".cachain.crt.pem", *args, **kwargs)
...
class CA():
    ...
    def run_openssl_with_config_option(self, config, command):
        with tempfile.NamedTemporaryFile(mode="w+t") as temp_file:
            utils.dumpconfig(config=config, file=temp_file)
            temp_file.flush()
    ...
    def genrsa(self, filename, cipher="aes256", numbits=4096):
        ...
        if cipher:
            command += ["-" + cipher]
        command += ["-out", filename, str(numbits)]
        self.run_command(command)
    ...
    def create_root_certificate(self,
        ...
        command = [
            "openssl", "req", "-key",
            pathname.get_keyfile(), "-new", "-x509", "-days",
            str(days), "-" + md, "-out",
    ...
    def create_intermediate_certificate_signing_request(self, pathname):
        self.run_command(["bash", "-c", "echo 1000 > crlnumber"])
        command = [
            "openssl", "req", "-new", "-sha256", "-key",
            pathname.get_keyfile(), "-out",
            pathname.get_csrfile()
        ]
        self.run_openssl_with_config_option(
            self.get_intermediate_config(pathname), command)

    def sign_intermediate_certificate(self,
                                      pathname,
                                      root_pathname,
                                      days=365,
                                      md="sha256",
        ...
        command = [
            "openssl", "ca", "-days",
            str(days), "-md", md, "-notext", "-in",
            pathname.get_csrfile(), "-out",
            pathname.get_crtfile()
        ]
        if extensions:
            command += ["-extensions", extensions]
    ...
    def sign_server_certificate(self,
                                ...
                                md="sha256"):
        command = [
            "openssl", "ca", "-extensions", "server_cert", "-days",
            str(days), "-notext", "-md", md, "-in",
            pathname.get_csrfile(), "-out",
            pathname.get_crtfile()
        ]
        self.run_openssl_with_config_option(
    ...
    def sign_client_certificate(self,
                                ...
                                md="sha256"):
        command = [
            "openssl", "ca", "-extensions", "usr_cert", "-days",
            str(days), "-notext", "-md", md, "-in",
            pathname.get_csrfile(), "-out",
            pathname.get_crtfile()
        ]
    ...
    def verify_certificate(self, pathname, ca_pathname):
        ...
        command = [
            "openssl", "verify", "-CAfile",
            ca_pathname.get_cachainfile(),
            pathname.get_crtfile()
        ]
        self.run_command(command)

    def create_certificate_revocation_list(self, pathname):
        command = ["openssl", "ca", "-gencrl", "-out", pathname.get_crlfile()]
        self.run_openssl_with_config_option(
            self.get_intermediate_config(pathname), command)
    ...
    def verify_certificate_revocation_list(self, pathname):
        command = [
            "openssl", "crl", "-in",
            pathname.get_crlfile(), "-noout", "-text"
        ]
        self.run_command(command)

    def revoke_certificate(self, pathname, ca_pathname):
        command = ["openssl", "ca", "-revoke", pathname.get_crtfile()]
        self.run_openssl_with_config_option(
            self.get_intermediate_config(ca_pathname), command)

    def run_ocsp_server(self,
                        pathname,
                        ca_pathname,
                        port="127.0.0.1:2560",
                        nrequest=None,
        ...
        command = [
            "openssl", "ocsp", "-port", port, "-text", "-" + md, "-index",
            ca_pathname.get_indexfile(), "-CA",
            ca_pathname.get_cachainfile(), "-rkey",
            pathname.get_keyfile(), "-rsigner",
            pathname.get_crtfile()
        ]
        if nrequest:
    ...
    def query_ocsp(self, pathname, ca_pathname, url="http://127.0.0.1:2560"):
        command = [
            "openssl", "ocsp", "-CAfile",
            ca_pathname.get_cachainfile(), "-url", url, "-resp_text",
            "-issuer",
            ca_pathname.get_crtfile(), "-cert",
            pathname.get_crtfile()
        ]
        self.run_command(command)
    ...
    def prepare_directory(self, pathname):
        ...
        os.chdir(prefix)
        self.run_command([
            "mkdir", "-p", pathname.dir_certs, pathname.dir_crl,
            pathname.dir_private, pathname.dir_csr, pathname.dir_newcerts
        ])
